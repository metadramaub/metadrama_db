...
export async function countAutoriaGroupsWithProposals(
	supabase: SupabaseClient<Database>,
	obraId: string,
	knownJornadaIds?: string[]
): Promise<number> {
	let jornadaIds = knownJornadaIds;
	if (!jornadaIds) {
		const jornadasResp = await supabase
			.from('jornadas')
			.select('jornada_id')
			.eq('obra_id', obraId);
		jornadaIds = (jornadasResp.data ?? []).map((row) => row.jornada_id);
	}

	const scopeFilter =
		jornadaIds.length > 0
			? `obra_id.eq.${obraId},jornada_id.in.(${jornadaIds.join(',')})`
			: `obra_id.eq.${obraId}`;
	...
	return new Set(
		(atribucionesResp.data ?? []).map(
			(row) => row.grupo_atribucion_id ?? `legacy:${row.atribucion_id}`
		)
	).size;
}
