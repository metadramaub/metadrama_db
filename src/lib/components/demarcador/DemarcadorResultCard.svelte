<script lang="ts">
	...
	const coincidencias = $derived(
		arquitectura.detalles.filter((detalle) => detalle.estado === 'coincide')
	);
	const contradicciones = $derived(
		arquitectura.detalles.filter((detalle) => detalle.estado === 'contradice')
	);
...
<li>
	<details class="group">
		<summary class="cursor-pointer list-none p-5 marker:hidden sm:p-6">
			...
			<div class="mt-4 grid gap-2 text-sm leading-5">
				{#if coincidencias.length > 0}
					<p>
						<strong class="font-semibold">Coincide:</strong>
						{coincidencias.slice(0, 2).map((detalle) => detalle.etiqueta).join(', ')}.
					</p>
				{/if}
				{#if contradicciones.length > 0}
					<p class="text-[color:var(--muted-foreground)]">
						<strong class="font-semibold text-[color:var(--foreground)]">Difiere:</strong>
						{contradicciones.slice(0, 2).map((detalle) => detalle.etiqueta).join(', ')}.
					</p>
				{/if}
			</div>

			<p class="mt-4 text-xs font-semibold text-[color:var(--muted-foreground)]">
				<span class="group-open:hidden">Ver detalles ↓</span>
				<span class="hidden group-open:inline">Ocultar detalles ↑</span>
			</p>
		</summary>

		<div class="border-t border-[color:var(--border)] px-5 pb-6 pt-5 sm:px-6">
			{#if forma.formaDefinicion}
				<p class="text-sm leading-6 text-[color:var(--gray-700)]">{forma.formaDefinicion}</p>
			{/if}
			...
			<a
				href={`/recursos/catalogo-metrico/${forma.formaSlug}`}
