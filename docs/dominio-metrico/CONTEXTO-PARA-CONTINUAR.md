...
| F13 | copla castellana **y novena-lira** | al elegir la rima no se ve desplegada verso a verso, en vertical, junto a cada verso, y queda lejos de donde se resume cada parte | UI | — | toda forma con grupo de rima | **3.ª vez → maduro**, va con **F6** |
...
| F34 | octava real · endecasilábica consonante | pregunta el **dístico final** siempre, y no aporta en ninguno de los dos caminos: el esquema habitual `ABABABCC` ya lo lleva, y si se escribe el esquema a mano la notación lo enseña | **catálogo** | el rasgo `distico_final` solo está en 2 arquitecturas —esta y el endecasílabo suelto, donde sí trabaja: allí no hay esquema que lo declare— | 1 arquitectura | **migración, sin aprobar** |
