...
| F29 | novena, **seguidilla gitana y las 6 de F44** | la medida se pide con un **desplegable** en vez de con las barras que dibujan el verso | UI | **causa localizada el 29 de agosto**: `MetricChoiceField` mira `maximum === 1` **antes** que las ramas posicionales, así que una pregunta de una sola posición cae en el desplegable aunque sea posicional. Le pasa desde siempre a la gitana y, desde que se declaró dónde cae el quiebro, también a las seis de **F44** | 7 arquitecturas hoy | recogido, va con **F14**; el arreglo es reordenar las ramas **y** dar al campo la medida que la norma fija donde no pregunta —la gitana no tiene rol `dominante` del que deducirla— |
