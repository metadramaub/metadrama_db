# Migración métrica · La tragedia del duque de Berganza

Generado el 2026-08-04 por `npm run migracion:informe`. **No editar a mano:**
se regenera y se pierde lo escrito. Las decisiones van a
[equivalencias pendientes](../equivalencias-pendientes.md).

- **Editor asignado:** María Rosa Álvarez Sellers
- **Secuencias métricas:** 5
- **Subtipos estróficos:** 0
- **Caracterizaciones por rango:** 0

## Nada que consultar

Todas las secuencias resuelven su equivalencia de forma directa.

Resolución: 4 directas · 1 con rasgo propio · 0 por ascendencia · 0 sin destino · 0 sin forma declarada.

## Secuencias

| Versos | v | Término actual | Forma propuesta | Arquitectura | Además | Vía |
| --- | ---: | --- | --- | --- | --- | --- |
| 1–64 | 64 | `redondilla` | Redondilla | Octosilábica | — | directa |
| 65–128 | 64 | `octava_real_regular` | Octava real | Endecasilábica consonante | — | directa |
| 129–184 | 56 | `redondilla` | Redondilla | Octosilábica | — | directa |
| 185–197 | 13 | `romance_a-e` | Romance | Octosílabo | Vocales de la asonancia = a-e | rasgo + forma del padre |
| 198–199 | 2 | `redondilla` | Redondilla | Octosilábica | — | directa |

