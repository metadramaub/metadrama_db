-- La ficha y el demarcador ven también las preguntas heredadas
--
-- Al sacar las preguntas heredadas al catálogo se reapuntaron las piezas del editor, y quedaron tres
-- que siguen leyendo la tabla base: las dos que arman la ficha pública y la que compila el
-- demarcador. **No fallaban** —la tabla no ha cambiado, y comprobado ejecutándolas las tres
-- responden— pero se quedaban ciegas: la rima de la oncena y la del septeto compuesto no aparecían
-- ni en la ficha ni en el artefacto.
--
-- Y el demarcador tenía además una incoherencia que este cambio le habría creado: **leía los grupos
-- de la vista resuelta y las opciones de la tabla base**, así que una pregunta heredada habría
-- salido sin ninguna respuesta que ofrecer.
--
-- Las tres pasan a leer `preguntas_metricas`. Son lecturas de «las preguntas de estas
-- arquitecturas»: no escriben nada y no cambian de forma.
--
-- **Tras esto hay que recompilar el artefacto del demarcador**, que es un precomputado.
...
begin
	-- ------------------------------------------------------------------ Comprobaciones
	--
	-- **Se ejecutan las tres.** Una función que compila puede reventar al llamarse, y estas arman
	-- la ficha pública entera.
	select jsonb_array_length(coalesce(public.get_forma_metrica_publica('oncena') -> 'arquitecturas', '[]'::jsonb))
	into v_arqs;

	if v_arqs < 1 then
		raise exception 'La ficha de la oncena no devuelve arquitecturas.';
	end if;

	perform public.get_forma_metrica_publica_jerarquica('oncena');
	perform public.obtener_catalogo_demarcador();

	-- Y ahora la oncena declara que su rima se elige, que es lo que no se veía.
	select count(*) into v_preguntas
	from public.preguntas_metricas g
	join public.arquitecturas_forma a on a.arquitectura_id = g.arquitectura_id
	join public.formas_metricas f on f.forma_id = a.forma_id
	where f.nombre = 'Oncena' and g.dimension = 'rima' and g.activo;

	if v_preguntas <> 4 then
		raise exception 'La oncena ofrece % preguntas de rima, y son 4: dos por arquitectura.', v_preguntas;
	end if;

	raise notice 'La ficha, la jerárquica y el demarcador responden, y la oncena ya pregunta su rima.';
end
$comprobacion$;
