-- Las formas se ordenan por lo que son, no por una columna muerta
--
-- C14. Se retira `formas_metricas.orden`, que **no la usaba nadie**, y en su lugar las dos funciones
-- públicas declaran **la extensión de la unidad de cada forma**, que es un orden que sí significa
-- algo.
--
-- **Nadie ordenaba por ella.** `get_catalogo_formas_publicas` y `get_forma_metrica_publica` la
-- seleccionaban y las dos ordenan sus formas `order by x.nombre`; los `order by x.orden` que hay
-- cerca son de **arquitecturas**, secciones y opciones, y esos sí trabajan —ponen la principal
-- delante— y no se tocan. El tipo público no la lleva y la página de `/formas` no la mira.
--
-- Y sus valores tampoco decían gran cosa: veintinueve formas tenían orden y **catorce no** —casi
-- todas las creadas en agosto—, de modo que el criterio, que parecía ser el número de versos
-- —redondilla 40, quintilla 50, sextilla 70, novena 90, copla real 110—, había quedado a medias. Y
-- tres formas compartían el 10.
--
-- **Lo que entra en su lugar.** `unidad_versos`, tomada de la **arquitectura principal** de cada
-- forma. Se elige la principal y no el mínimo de todas porque es la realización canónica: la décima
-- son diez versos aunque su *aumentada* mida doce, y la seguidilla cuatro aunque la chamberga mida
-- diez. Son las tres excepciones que
-- [criterios de nivel § 3.1](../../docs/dominio-metrico/criterios-de-nivel.md) enumera, y ninguna
-- deja de ser lo que es por tener una arquitectura más larga.
--
-- Sale nulo donde **no hay unidad**: las siete series y las composiciones de extensión variable
-- —canción, villancico, zéjel—. No es un hueco: una serie no tiene número de versos, y ordenar por
-- él tiene que dejarlas fuera de la cuenta en vez de fingir un cero.
--
-- **Dos tropiezos de esta migración, anotados porque los dos se repiten solos.** El primero:
-- transcribir a mano la cola de una función para cambiarle tres líneas se inventó columnas y perdió
-- una clave entera; se arma desde `pg_get_functiondef` y se sustituye solo el bloque que cambia. El
-- segundo: la guarda comprobaba quién **ordenaba** por la columna y no quién la **seleccionaba**, y
-- se dejó fuera la ficha, que la seleccionaba sin usarla. Ahora comprueba las dos cosas.
...
begin
	if not exists (
		...
		where table_schema = 'public' and table_name = 'formas_metricas' and column_name = 'orden'
	) then
		raise exception 'La columna formas_metricas.orden ya no está.';
	end if;

	-- **Quién la selecciona**, que es lo que hay que reescribir, y **quién ordena por ella**, que
	-- sería lo que impediría retirarla. Las dos preguntas, porque la primera versión solo hizo la
	-- segunda y se dejó la ficha fuera.
	for v_funcion in
		select proname from pg_proc
		where prosrc ~ 'nivel_estructural, orden'
	loop
		if v_funcion not in ('get_catalogo_formas_publicas', 'get_forma_metrica_publica') then
			raise exception 'La función «%» también selecciona el orden de las formas.', v_funcion;
		end if;
	end loop;

	select count(*) into v_n
	from pg_proc where prosrc ~ 'nivel_estructural, orden';
	if v_n <> 2 then
		raise exception '% funciones seleccionan el orden de las formas, y se esperaban 2.', v_n;
	end if;

	select count(*) into v_n
	from pg_proc where prosrc ~ 'order by[^;]*\mf\.orden';
	if v_n <> 0 then
		raise exception '% funciones ordenan formas por esa columna; no se puede retirar.', v_n;
	end if;

	-- ------------------------------ El listado, sin la columna y con la extensión
	-- Copiado de la definición viva; solo cambia el bloque de las formas.
		...
		select jsonb_build_object(
			'formas', coalesce((
				...
				from (
					...
					from public.formas_metricas x0
					where x0.activo
				) x
			), '[]'::jsonb),
...
;
	...
	-- ------------------------------- Y la ficha, que la seleccionaba igual
...
;
	...
	alter table public.formas_metricas drop column orden;

	-- ------------------------------------------------------------------ Comprobaciones
	-- Ejecutando las dos funciones, no leyendo sus cuerpos. Las claves del listado, una a una:
	-- reescribir una función copiándola es la manera clásica de perder una por el camino.
	foreach v_clave in array array[
		'formas', 'arquitecturas', 'tiposRima', 'denominaciones', 'tradiciones', 'formasTradiciones'
	] loop
		if not public.get_catalogo_formas_publicas() ? v_clave then
			raise exception 'El listado público ha perdido la clave «%».', v_clave;
		end if;
	end loop;
	if (select count(*) from jsonb_object_keys(public.get_catalogo_formas_publicas())) <> 6 then
		raise exception 'El listado público devuelve un número de claves distinto de seis.';
	end if;

	select jsonb_array_length(public.get_catalogo_formas_publicas() -> 'formas') into v_n;
	if v_n < 40 then
		raise exception 'El listado público devuelve % formas, y son más de cuarenta.', v_n;
	end if;

	-- La ficha responde y trae lo suyo: sus claves son muchas más y se comprueban por número.
	if public.get_forma_metrica_publica('soneto') -> 'formas' = '[]'::jsonb then
		raise exception 'La ficha del soneto ha dejado de responder.';
	end if;
	if jsonb_array_length(public.get_forma_metrica_publica('soneto') -> 'arquitecturas') = 0 then
		raise exception 'La ficha del soneto ha perdido sus arquitecturas.';
	end if;

	-- El soneto declara sus catorce en las dos superficies; el romance, que es serie, ninguno.
	if (
		select x ->> 'unidad_versos'
		from jsonb_array_elements(public.get_catalogo_formas_publicas() -> 'formas') x
		where x ->> 'slug' = 'soneto'
	) is distinct from '14' then
		raise exception 'El soneto no declara sus catorce versos en el listado.';
	end if;
	if (
		select x ->> 'unidad_versos'
		from jsonb_array_elements(public.get_forma_metrica_publica('soneto') -> 'formas') x
		where x ->> 'slug' = 'soneto'
	) is distinct from '14' then
		raise exception 'El soneto no declara sus catorce versos en su ficha.';
	end if;
	if (
		select x ->> 'unidad_versos'
		from jsonb_array_elements(public.get_catalogo_formas_publicas() -> 'formas') x
		where x ->> 'slug' = 'romance'
	) is not null then
		raise exception 'El romance declara un número de versos, y es una serie.';
	end if;

	-- La décima son diez aunque su aumentada mida doce, y la seguidilla cuatro aunque su chamberga
	-- mida diez: manda la principal, no el máximo. Son dos de las tres excepciones del § 3.1.
	if (
		select x ->> 'unidad_versos'
		from jsonb_array_elements(public.get_catalogo_formas_publicas() -> 'formas') x
		where x ->> 'slug' = 'decima'
	) is distinct from '10' then
		raise exception 'La décima no declara diez versos: la principal no está mandando.';
	end if;
	if (
		select x ->> 'unidad_versos'
		from jsonb_array_elements(public.get_catalogo_formas_publicas() -> 'formas') x
		where x ->> 'slug' = 'seguidilla'
	) is distinct from '4' then
		raise exception 'La seguidilla no declara cuatro versos.';
	end if;
	...
	if exists (
		...
		raise exception 'La columna sigue ahí.';
	end if;

	-- Y las otras dos superficies no se han enterado.
