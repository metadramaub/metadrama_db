-- La ficha dice tambien lo que se anoto y no se veia
--
-- Tres cosas, todas sobre la misma funcion.
--
-- 1. Una secuencia pertenece al cuadro donde EMPIEZA. Antes se exigia que cupiera entera dentro del
--    cuadro, y como el tablado se vacia muchas veces en mitad de una tirada -el 31 % de los cambios
--    de cuadro de Fuente Ovejuna y de Peribanez caen ahi-, esas secuencias salian sin cuadro. En las
--    obras de prueba eran 6 de las 33 de una de ellas. Que la tirada siga sonando despues del corte
--    va aparte, en `cuadro_continua`, porque es un dato y no un estorbo.
--
-- 2. Los esquemas de rima de una tirada se cuentan en vez de enumerarse. El esquema se responde una
--    vez por estrofa, asi que una tirada de setenta y ocho redondillas traia setenta y ocho entradas
--    y el codigo de barras se llenaba de rayas.
--
-- 3. Salen los rasgos, los metros y las desviaciones, que estaban anotados y no llegaban a ninguna
--    parte: cincuenta y siete asonancias de romance, las lagunas, los versos hipometricos.
...
-- La funcion se ejecuta, que es la unica manera de saber que compila y devuelve lo que dice.
do $guarda$
declare
  v_obra uuid;
  v_admin uuid;
...
begin
  ...
  if v_admin is null then
    raise notice 'Sin editor admin: no se puede ejecutar la ficha en la guarda.';
    return;
  end if;
  ...
  join public.anotaciones_metricas a on a.secuencia_id = sm.secuencia_id
  join public.anotacion_elecciones e on e.anotacion_id = a.anotacion_id
  where e.valor_rasgo_id is not null
  group by sm.obra_id
  order by count(*) desc
  limit 1;
  if v_obra is null then
    raise notice 'No hay ninguna obra con rasgos anotados: nada que comprobar.';
    return;
  end if;

  v_ficha := public.get_obra_ficha_publica_base_without_slugs(v_obra, true);
  if v_ficha is null then
    raise exception 'La ficha devolvio nulo para la obra %', v_obra;
  end if;

  select
    count(*) filter (where s->>'cuadro_id' is null),
    count(*) filter (where jsonb_array_length(s->'rasgos') > 0)
  into v_sin_cuadro, v_con_rasgos
  from jsonb_array_elements(v_ficha->'metrica'->'secuencias') s;

  if v_sin_cuadro > 0 then
    raise exception 'Quedan % secuencias sin cuadro en la obra %', v_sin_cuadro, v_obra;
  end if;
  if v_con_rasgos = 0 then
    raise exception 'La obra % tiene rasgos anotados y la ficha no devuelve ninguno', v_obra;
  end if;

  raise notice 'Ficha comprobada sobre %: ninguna secuencia sin cuadro, % con rasgos.', v_obra, v_con_rasgos;
end;
$guarda$;
