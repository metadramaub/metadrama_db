-- La redondilla dice dónde cayó su quiebro
--
-- Paso 8c de B2, una de siete. Toca 1 arquitectura de 4 versos.
--
-- **El caso que B2 puso de ejemplo.** El pie quebrado se preguntaba en la copla real y no en la
-- redondilla, teniendo las dos el mismo rasgo con la misma modalidad. Navarro Tomás documenta la
-- redondilla quebrada en los cancioneros del siglo XV, cruzada y abrazada, y **sin fijar en qué
-- versos cae el quiebro**: por eso el editor tiene que poder decirlo, y por eso `posiciones_max` se
-- queda vacío —regla 5 bis del § 3.6—.
--
...
begin
	...
	if v_forma is null then
		raise exception 'La forma «redondilla» no está activa.';
	end if;
	...
	if public.get_forma_metrica_publica('redondilla') -> 'formas' = '[]'::jsonb then
		raise exception 'La ficha de redondilla ha dejado de responder.';
	end if;
end $$;
