alter table public.secuencias_metricas
	add column if not exists evocacion_metrica boolean not null default false,
	add column if not exists evocacion_metrica_texto text null;

comment on column public.secuencias_metricas.evocacion_metrica is
	'Marca si la secuencia corresponde a un caso de evocación métrica de la voz de otro personaje.';

comment on column public.secuencias_metricas.evocacion_metrica_texto is
	'Explicación breve del caso de evocación métrica.';
