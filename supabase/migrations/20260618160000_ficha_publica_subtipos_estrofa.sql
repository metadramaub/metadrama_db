-- Fase 6: añade los subtipos de estrofa por secuencia al payload de la ficha
-- pública (get_obra_ficha_publica). Hoy no se exponían.
--
-- Patrón idéntico al de caracterizaciones_by_secuencia: una CTE que agrega los
-- subtipos por secuencia, y un nuevo campo 'subtipos_estrofa' en cada secuencia.
-- El resto de la función queda igual.

CREATE OR REPLACE FUNCTION public.get_obra_ficha_publica(p_obra_id uuid, p_include_hidden boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
				...
				coalesce(
					jsonb_agg(
						jsonb_build_object(
							...
							'subtipo_estrofa_term', coalesce(sv.termino, 'sin_subtipo'),
							'v_ini', sse.v_ini,
							'v_fin', sse.v_fin
						)
