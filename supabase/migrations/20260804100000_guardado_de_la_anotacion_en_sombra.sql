-- Fase 0, segunda mitad: el guardado acepta una secuencia real.
--
-- Hasta ahora la función exigía un escenario ficticio: el `update` comprobaba la existencia
-- del escenario y sin él no encontraba nunca la fila. Ahora una prueba se guarda contra un
-- escenario o contra una secuencia real, y nunca contra las dos cosas.
--
-- Lo que no cambia: el árbol nuevo —realizaciones, elecciones, desviaciones— sigue colgando
-- de la prueba. `secuencias_metricas` no se toca ni se lee para escribir en ella.
--
...
comment on function public.guardar_secuencia_editor_metrico_prueba(jsonb) is
	'Guarda una prueba del editor V2, colgada de un escenario ficticio o de una secuencia real (anotación en sombra). En el segundo caso el rango lo manda la secuencia real y la obra debe estar abierta en obras_editor_metrico_v2.';
