-- La copla castellana dice dónde cayó su quiebro
--
-- Paso 8c de B2, cuatro de siete. Toca 1 arquitectura de 8 versos.
--
-- La copla castellana declara el quiebro como admitido y no dejaba decir en qué verso cayó.
--
-- *Su nota documenta también la **alternancia** de versos plenos y quebrados, y eso no significa uno
-- de cada dos: significa unos sí y otros no.* De ahí que no haya techo que sacar de la fuente y que
-- `posiciones_max` se quede vacío, con el límite de registro derivado de la unidad menos uno.
--
...
begin
	...
	if v_nota is null or v_nota not like '%tetrasílabo%' then
		raise exception 'La nota del quiebro de copla_castellana no es la que se esperaba: «%».', v_nota;
	end if;

	update public.arquitectura_rasgos ar
	set nota = 'El quiebro que documentan las fuentes es tetrasílabo y suele ser uno solo. Se documenta en el sexto verso, y también alternando versos plenos y quebrados en las dos semiestrofas. El catálogo admite además el pentasílabo, que el *Diccionario* cuenta como el mismo quebrado: «si el verso tetrasílabo tiene cinco sílabas, hay que tener en cuenta la sinafía y compensación entre versos».'
	...
	if v_n = 0 then
		raise exception 'La nota del quiebro de copla_castellana no recoge el pentasílabo.';
	end if;
