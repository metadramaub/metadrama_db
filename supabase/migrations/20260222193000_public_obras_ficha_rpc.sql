...
alter table public.obras
	add column if not exists autor_ficha_publico text;

update public.obras o
set autor_ficha_publico = e.nombre_completo
from public.editores e
where o.editor_asignado = e.user_id
	and o.autor_ficha_publico is distinct from e.nombre_completo;

update public.obras
set autor_ficha_publico = null
where editor_asignado is null
	and autor_ficha_publico is not null;

create or replace function public.sync_obra_autor_ficha_publico()
returns trigger
language plpgsql
security definer
...
drop trigger if exists trigger_obras_sync_autor_ficha_publico on public.obras;
create trigger trigger_obras_sync_autor_ficha_publico
before insert or update of editor_asignado on public.obras
for each row
execute function public.sync_obra_autor_ficha_publico();
	...
	return (
		...
		select jsonb_build_object(
			...
			jsonb_build_object(
				...
				'analisis_editor', v_obra.analisis_editor,
