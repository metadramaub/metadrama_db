-- La copla de arte menor dice dónde cayó su quiebro
--
-- Paso 8c de B2, cinco de siete. Toca 1 arquitectura de 8 versos.
--
-- Salió de la `redondilla · doble_enlazada` el 21 de agosto de 2026 con el quiebro admitido y sin
-- pregunta. Mismo caso que su hermana la castellana, y con la misma alternancia documentada.
--
...
begin
	...
	if v_nota is null or v_nota not like '%tetrasílabo%' then
		raise exception 'La nota del quiebro de copla_de_arte_menor no es la que se esperaba: «%».', v_nota;
	end if;

	update public.arquitectura_rasgos ar
	set nota = 'El quiebro que documentan las fuentes es tetrasílabo. Se documenta en semiestrofas alternando versos plenos y quebrados, y también con un solo verso corto en la segunda mitad. El catálogo admite además el pentasílabo, que el *Diccionario* cuenta como el mismo quebrado: «si el verso tetrasílabo tiene cinco sílabas, hay que tener en cuenta la sinafía y compensación entre versos».'
	...
	if v_n = 0 then
		raise exception 'La nota del quiebro de copla_de_arte_menor no recoge el pentasílabo.';
	end if;
