...
begin
	if exists (
		...
		where table_schema = 'public'
			...
			and column_name = 'personaje_femenino'
	) then
		alter table public.secuencias_metricas
			rename column personaje_femenino to intervencion_personajes_femeninos;
	end if;
	...
	if exists (
		...
		where table_schema = 'public'
			and table_name = 'secuencias_metricas'
			and column_name = 'personajes_donaire'
	) then
		alter table public.secuencias_metricas
			rename column personajes_donaire to intervencion_figuras_donaire;
	end if;
	...
	if exists (
		...
		where table_schema = 'public'
			and table_name = 'secuencias_metricas'
			and column_name = 'personajes_sobrenatural'
	) then
		alter table public.secuencias_metricas
			rename column personajes_sobrenatural to intervencion_personajes_sobrenaturales;
	end if;
end $$;

alter table public.secuencias_metricas
	drop constraint if exists secuencias_metricas_personaje_femenino_chk,
	drop constraint if exists secuencias_metricas_intervencion_personajes_femeninos_chk,
	drop constraint if exists secuencias_metricas_intervencion_figuras_donaire_chk,
	drop constraint if exists secuencias_metricas_intervencion_personajes_sobrenaturales_chk;

update public.secuencias_metricas
set
	intervencion_personajes_femeninos = case intervencion_personajes_femeninos
		when 'ausente' then 'sin_intervencion'
		when 'solo' then 'exclusiva'
		when 'con_otros' then 'compartida'
		else coalesce(intervencion_personajes_femeninos, 'sin_intervencion')
	end,
	intervencion_figuras_donaire = case intervencion_figuras_donaire
		when 'ausente' then 'sin_intervencion'
		when 'solo' then 'exclusiva'
		when 'con_otros' then 'compartida'
		else coalesce(intervencion_figuras_donaire, 'sin_intervencion')
	end,
	intervencion_personajes_sobrenaturales = case intervencion_personajes_sobrenaturales
		when 'ausente' then 'sin_intervencion'
		when 'solo' then 'exclusiva'
		when 'con_otros' then 'compartida'
		else coalesce(intervencion_personajes_sobrenaturales, 'sin_intervencion')
	end;

alter table public.secuencias_metricas
	alter column intervencion_personajes_femeninos set default 'sin_intervencion',
	alter column intervencion_figuras_donaire set default 'sin_intervencion',
	alter column intervencion_personajes_sobrenaturales set default 'sin_intervencion';

alter table public.secuencias_metricas
	add constraint secuencias_metricas_intervencion_personajes_femeninos_chk
		check (intervencion_personajes_femeninos in ('sin_intervencion', 'exclusiva', 'compartida')),
	add constraint secuencias_metricas_intervencion_figuras_donaire_chk
		check (intervencion_figuras_donaire in ('sin_intervencion', 'exclusiva', 'compartida')),
	add constraint secuencias_metricas_intervencion_personajes_sobrenaturales_chk
		check (intervencion_personajes_sobrenaturales in ('sin_intervencion', 'exclusiva', 'compartida'));

comment on column public.secuencias_metricas.intervencion_personajes_femeninos is
	'Indica si en esta secuencia métrica intervienen verbalmente personajes femeninos.';

comment on column public.secuencias_metricas.intervencion_figuras_donaire is
	'Indica si en esta secuencia métrica intervienen verbalmente figuras de donaire.';

comment on column public.secuencias_metricas.intervencion_personajes_sobrenaturales is
	'Indica si en esta secuencia métrica intervienen verbalmente personajes sobrenaturales.';
