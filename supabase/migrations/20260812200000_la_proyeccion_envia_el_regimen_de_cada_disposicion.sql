-- La proyección envía el régimen de cada disposición.
--
-- `esquemas_rima.tipo_rima_id` está poblado en las 87 disposiciones y la proyección pública no lo
-- mandaba, así que la ficha leía «sin declarar» justo en las tres arquitecturas que lo declaran
-- ahí a propósito: las dos del villancico —`abba` y `abab` consonantes junto a la asonantada
-- `abcb`— y la canción sin rima, cuyo cuerpo no rima y cuyo pareado final sí. Es la clave que
-- faltaba para que el criterio de `D15` se pueda leer en pantalla y no solo comprobar.
--
-- Solo añade una columna a una clave que ya viajaba: lo que `main` lee de ella sigue estando.
...
begin
	...
	from jsonb_array_elements(
		public.get_forma_metrica_publica_jerarquica('villancico') -> 'esquemasRima'
	) e
	where e ->> 'tipo_rima_id' is null;
	if sin_tipo > 0 then
		raise exception '% disposiciones del villancico llegan sin su régimen de rima', sin_tipo;
	end if;
end;
$$;
