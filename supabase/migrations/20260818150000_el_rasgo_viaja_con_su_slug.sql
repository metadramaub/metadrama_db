-- El rasgo viaja con su slug
--
-- La ficha necesita saber si una arquitectura declara la densidad de su rima, porque es una de
-- las tres maneras que tiene de decir cómo se comporta —las otras dos son las restricciones de
-- un esquema abierto y unos esquemas concretos de los que se calcula—. Sin el `slug`, la única
-- forma de reconocer ese rasgo era comparar su nombre visible, «Densidad de rima», y un
-- criterio que decide si algo se enseña en rojo no puede depender de una cadena de pantalla.
--
-- La función se reescribe entera porque `create or replace` lo exige; se generó de la
-- definición viva con ese único cambio.
...
declare
	...
	v_slugs integer;
begin
	v_payload := public.get_forma_metrica_publica_jerarquica('sextilla');

	select count(*) into v_slugs
	from jsonb_array_elements(v_payload->'rasgos') r
	where r->>'slug' is not null;

	if v_slugs = 0 then
		raise exception 'Los rasgos de la ficha siguen llegando sin slug.';
	end if;
end $$;
