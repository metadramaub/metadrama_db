-- La septilla dice dónde cayó su quiebro
--
-- Paso 8c de B2, tres de siete. Toca 1 arquitectura de 7 versos.
--
-- La septilla entró el 21 de agosto de 2026 declarando el quiebro como admitido y sin pregunta que
-- lo recogiera.
--
-- *Su nota dice que se documenta en el quinto verso, y eso **no** fija un techo:* dice dónde, no
-- cuántos como máximo, y derivar un máximo de un ejemplo es convertir la muestra en ley. Es la regla
-- 5 bis del § 3.6, y por eso `posiciones_max` se queda vacío.
--
...
begin
	...
	if v_nota is null or v_nota not like '%tetrasílabo%' then
		raise exception 'La nota del quiebro de septilla no es la que se esperaba: «%».', v_nota;
	end if;
	...
	if v_n = 0 then
		raise exception 'La nota del quiebro de septilla no recoge el pentasílabo.';
	end if;
