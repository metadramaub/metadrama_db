...
-- > tampoco el máximo.
--
-- En el orden 4+5 la nota nombra el verso: «se documenta abriéndola: **el quinto verso de la
-- estrofa**». Uno.
--
-- **En el orden 5+4 hay un matiz, y conviene que conste.** La nota no nombra ningún verso —«el
-- quiebro pasa a la redondilla, con la quintilla en octosílabos plenos»—, así que la posición no se
-- sabe. Lo que sí dice es **cuántos**: habla de *el* quiebro, en singular, igual que su hermana, y
-- declara el otro miembro en octosílabos plenos. El techo sale del número, no de la posición.
--
...
begin
	...
	if v_forma is null or v_rasgo is null then
		raise exception 'Falta la forma «novena» o el rasgo del quiebro.';
	end if;

	foreach v_par slice 1 in array array[
		array['redondilla_quintilla', '1'],
		array['quintilla_redondilla', '1']
	] loop
		select arquitectura_id into v_arq
		from public.arquitecturas_forma
		where forma_id = v_forma and slug = v_par[1] and activo;
		if v_arq is null then
			raise exception 'La arquitectura «%» de novena no está activa.', v_par[1];
		end if;
	...
	if public.get_forma_metrica_publica('novena') -> 'formas' = '[]'::jsonb then
		raise exception 'La ficha de novena ha dejado de responder.';
	end if;
end $$;
