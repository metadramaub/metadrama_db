-- Expone el slug crudo de la forma raíz (estrofa_forma_slug / forma_slug) y
-- su tipo_forma en el payload público, para colorear las formas métricas
-- por una clave estable (independiente de la etiqueta editable) en ficha,
-- catálogo y perfiles de autor.
--
-- Solo recrea get_obra_ficha_publica; no toca tablas ni datos.
--
-- Esta red bloquea el puerto de Postgres: aplicar por el SQL Editor del panel
-- (HTTPS/443) y registrar abajo en schema_migrations con un INSERT manual.
...
values ('20260618190000', 'ficha_forma_color_slug')
