-- El guardado nombra la pregunta por su enunciado derivado.
--
-- `guardar_secuencia_editor_metrico_prueba` recorre los grupos con `select *` de la tabla y usa
-- `v_grupo.nombre` en los tres mensajes de error que le dicen al editor qué pregunta está mal
-- respondida. Al derivarse el enunciado, la tabla dejó de tener esa columna.
--
-- Es un fallo especialmente incómodo porque **el camino feliz seguía funcionando**: solo habría
-- reventado al avisar de un fallo, es decir, exactamente cuando el editor necesita el aviso.
-- PL/pgSQL resuelve los campos de un `record` en ejecución, así que ni la migración ni las
-- pruebas dijeron nada.
--
-- Se rehace apuntando los tres recorridos a la vista, que es donde vive ahora el enunciado.
...
declare
	v_n integer;
begin
	-- Ninguna función puede seguir leyendo los grupos y esperando un `nombre` que la tabla ya no
	-- tiene. La vista sí lo da, y es de donde deben tomarlo.
	select count(*) into v_n
	from pg_proc p join pg_namespace n on n.oid = p.pronamespace
	where n.nspname = 'public' and p.prokind = 'f'
		and pg_get_functiondef(p.oid) ~ 'from public\.grupos_eleccion_metrica[^_]'
		and pg_get_functiondef(p.oid) ~ '(grupo|g)\.nombre';
	if v_n <> 0 then
		raise exception '% funciones leen los grupos de la tabla y esperan su nombre', v_n;
	end if;
end;
$$;
