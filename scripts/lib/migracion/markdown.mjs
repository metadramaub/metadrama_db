...
import { ordenDeConfirmaciones } from './modelo.mjs';
...
export function informeDeObra(obra, fecha) {
	...
	if (respuestas.length > 0) {
		...
		for (const r of respuestas) {
			w(
				`| ${r.versos} | ${celda(r.forma)} | ${celda(r.asunto)} | ${celda(r.formato?.ayuda ?? '')} |`
			);
		}
		w();
	}
	...
	if (confirmaciones.length > 0) {
		...
		w('| --- | --- | --- | --- | --- |');
		for (const c of ordenDeConfirmaciones(confirmaciones)) {
			w(
